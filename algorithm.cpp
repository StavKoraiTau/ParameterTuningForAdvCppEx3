...
REGISTER_ALGORITHM(ParameterAlgorithm);

double ParameterAlgorithm::getValidScore(const AlgoInfo& state,
...
std::optional<double> ParameterAlgorithm::getCloseValidScore(const AlgoInfo& state,
                                                             const AlgoTileInfo& tile_info) const {
    double score = 0;
    double batt_precent = static_cast<double>(state.getBattery()) / state.getMaxBattery();
    if (tile_info.getStatus() == TileStatus::ValidUnvisited) {
